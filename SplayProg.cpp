...
SplayTreeInventory::into(string id, int amount)
{
	...
	Node* N = stmap->findSplay(id);
	if (N != nullptr && N->key == id)
		stmap->putSplay(id, (N->value += amount));
	else
...
//this calcSize Function will take in a node and see if it is empty then add its value to its children recursivly
int	SplayTreeInventory::calcSize(Node* r)
{
	int sum = 0;
	if (!r)
		return 0;
	else
		sum += r->value + calcSize(r->right) + calcSize(r->left);
		return sum; 
}
...
{
	//this fuction will call the calcSize function and start from the root
	return calcSize(this->stmap->root);
}
...
//function that prints all children and grandchildren of a node recursevily 
void SplayTreeInventory::PrintRec(Node* s)
{
	if (s == NULL)
	{
		return;
